...
phpcbf:
	test -f /tmp/phpcs || curl -OL https://squizlabs.github.io/PHP_CodeSniffer/phpcs.phar > /tmp/phpcs && chmod +x /tmp/phpcs
	test -f /tmp/phpcbf || curl -OL https://squizlabs.github.io/PHP_CodeSniffer/phpcbf.phar > /tmp/phpcbf && chmod +x /tmp/phpcbf
	test -d /tmp/wpcs || git clone -b master --depth 1 https://github.com/WordPress-Coding-Standards/WordPress-Coding-Standards.git /tmp/wpcs
	/tmp/phpcs --config-set installed_paths /tmp/wpcs
